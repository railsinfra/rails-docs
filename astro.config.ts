    ...
    stainlessDocs({
      ...
      head: [
        ...
        },
        {
          ...
          tag: 'script',
          content: `(function(){var LANG_KEY='stldocs-selected-language';var BASE='/api';var LANGS=['typescript','python','go','java','kotlin','csharp','ruby','php'];function safeLang(value){if(!value){return''}var lower=String(value).toLowerCase();return lower==='node'?'typescript':lower}function replace(pathname){window.location.replace(pathname+window.location.search+window.location.hash)}function redirect(){var p=window.location.pathname;if(p!==BASE&&!p.startsWith(BASE+'/')){return}var rest=p.slice(BASE.length);var seg=safeLang((rest.split('/')[1])||'');if(seg){var segmentPath='/' + ((rest.split('/')[1])||'');if(segmentPath!==('/'+seg)){replace(BASE+'/'+seg+rest.slice(segmentPath.length));return}if(LANGS.indexOf(seg)!==-1){return}}var lang='';try{lang=safeLang(localStorage.getItem(LANG_KEY))}catch(e){}if(!lang||LANGS.indexOf(lang)===-1){lang='typescript'}replace(BASE+'/'+lang+rest)}redirect();document.addEventListener('astro:page-load',redirect)})();`,
        },
        {
          tag: 'script',
